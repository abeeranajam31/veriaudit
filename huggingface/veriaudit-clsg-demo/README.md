...
Pick a pilot test intent, inspect its four linguistic variants, and run a
demo evaluation to see the resulting CLSG. This is a static (client-side
only) Space — the same DEMO MODE logic used at
[veriaudit.vercel.app/platform](https://veriaudit.vercel.app/platform),
ported to plain JS so it needs no server-side compute.

⚠️ **DEMO MODE.** Model scores are deterministic, illustrative placeholders
(no real model is queried). Linguistic variants are the human-authored
